...
interface CodingInterviewWorkspaceProps {
  ...
  onChangeTopic?: () => void | Promise<void>;
}
...
export default function CodingInterviewWorkspace({
  ...
  const [selectedLanguage, setSelectedLanguage] =
    useState<CodingLanguage>(initialLanguage);
  ...
  useEffect(() => {
    ...
    const savedHistory = localStorage.getItem(
      `coding-submission-history-${interviewId}`,
    );
  ...
  useEffect(() => {
    ...
    setCode(
      activeQuestion.starterCode[selectedLanguage] ??
        activeQuestion.starterCode.javascript ??
        "",
    );
  ...
  const setAttempted = (questionId: string, solved: boolean) => {
    setQuestionStatus((prev) => ({
      ...
      [questionId]: {
        attempted: true,
        solved: Boolean(prev[questionId]?.solved || solved),
      },
    }));
  };
  ...
  const execute = async (mode: "run" | "submit") => {
    ...
    try {
      ...
      setOutput(result);
      setAttempted(activeQuestion.id, result.status === "Accepted");
      ...
      setOutput({
        ...
        aiFeedback: {
          ...
          explanation:
            "Could not evaluate this submission due to an API error.",
        },
      });
    } finally {
      setRunningMode(null);
      ...
      <div className="relative left-1/2 right-1/2 w-screen -translate-x-1/2 overflow-x-hidden px-4 pb-16 pt-20 sm:px-6 lg:px-8">
        ...
        <div className="interview-glass p-8 text-center">
          ...
          <p className="mt-2 text-light-400">
            Create a new coding interview to begin a coding session.
          </p>
        </div>
      </div>
    );
      ...
      <div className="w-full overflow-x-hidden space-y-6 px-4 pb-6 pt-16 sm:px-6 lg:px-8">
        ...
        <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-gradient-to-br from-slate-900/95 via-slate-900/80 to-blue-950/35 p-6">
          ...
          <div className="relative z-10 grid gap-5 lg:grid-cols-[1.45fr_0.95fr] lg:items-stretch">
            ...
            <div className="grid h-full grid-rows-[auto_1fr] gap-3 lg:justify-items-end">
              {selectedTopicLabel && onChangeTopic ? (
                <div className="flex w-44 flex-col items-stretch gap-1.5">
                  ...
                  <p className="text-center text-[11px] text-light-400">
                    Switch to a different topic set
                  </p>
                </div>
              ) : (
                <div className="inline-flex w-44 items-center justify-center rounded-xl border border-emerald-400/30 bg-emerald-500/12 px-4 py-2.5 text-sm font-medium text-emerald-300">
          ...
          <div className="mt-6">
            ...
            <p className="mt-2 text-xs text-light-400">
              Attempted {attemptedCount}/{questions.length} questions
            </p>
          </div>
        </div>

        <div className="grid w-full gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 xl:justify-items-start">
          {questions.map((question, index) => {
            const status = questionStatus[question.id];
            const wasAccepted = submissionHistory.some(
              (entry) =>
                entry.questionId === question.id && entry.status === "Accepted",
            );
            const isSolved = Boolean(status?.solved || wasAccepted);
            const isAttempted = Boolean(status?.attempted || wasAccepted);
            ...
            const cardStateClasses = isSolved
            ...
            return (
              <button
                ...
                className={`interview-glass group w-full max-w-[280px] border p-5 text-left transition-all duration-200 hover:-translate-y-1 ${cardStateClasses}`}
              >
                <div className="mb-3 flex items-center justify-between">
                  <span className="text-xs font-semibold text-light-400">
                    Question {index + 1}
                  </span>
                ...
                <div className="mt-3 flex items-center justify-between">
                  <span
                    className={`rounded-full px-2 py-0.5 text-xs ${difficultyClasses}`}
                  >
                    {question.difficulty}
                  </span>
  ...
  const languageMonaco =
    CODING_LANGUAGES.find((lang) => lang.value === selectedLanguage)?.monaco ??
    "javascript";
  const outputLineCount = (output?.output ?? "").split("\n").length;
  const shouldScrollOutput = outputLineCount > 5;

  return (
    <div className="relative left-1/2 right-1/2 h-screen w-screen -translate-x-1/2 overflow-x-hidden px-2 pb-3 pt-16 sm:px-3 lg:px-4 flex min-h-0 flex-col">
      {renderCodingNavbar()}
        ...
        <section className="interview-glass flex min-h-0 flex-col w-full lg:w-[var(--left-panel-width)]">
          <div className="border-b border-white/8 p-4">
            ...
            <div className="mt-4 flex items-center gap-2">
              {(["description", "hints", "editorial"] as LeftTab[]).map(
                (tab) => (
                  <button
                    key={tab}
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {activeQuestion.constraints.map((constraint) => (
                      <li
                        key={constraint}
                        className="rounded-lg bg-white/5 px-3 py-2 text-xs text-light-300"
                      >
                        {constraint}
                      </li>
                    ))}
            ...
            {leftTab === "hints" && (
              <div className="space-y-3">
                {activeQuestion.hints.map((hint, idx) => (
                  <div
                    key={hint}
                    className="rounded-xl border border-primary-200/20 bg-primary-200/10 p-3"
                  >
              ...
              <div className="space-y-4">
                ...
                <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-4">
                  ...
                  <p className="text-sm text-light-100">
                    Time: {activeQuestion.timeComplexity}
                  </p>
                  <p className="text-sm text-light-100">
                    Space: {activeQuestion.spaceComplexity}
                  </p>
                </div>
              </div>
            )}
        ...
        <section className="interview-glass flex min-h-0 flex-col overflow-hidden w-full lg:w-[calc(100%-var(--left-panel-width)-0.5rem)]">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-white/8 p-3">
            <div className="flex items-center gap-2">
              ...
              <label htmlFor="coding-language-select" className="sr-only">
              ...
              <select
                ...
                onChange={(e) =>
                  setSelectedLanguage(e.target.value as CodingLanguage)
                }
          ...
          <div
            className={`min-h-0 flex-1 ${isBottomOpen ? "border-b border-white/8" : ""}`}
          >
            <MonacoEditor
              height="100%"
          ...
          <div
            className={
              isBottomOpen
                ? "h-[250px] md:h-[280px] lg:h-[300px] overflow-hidden"
                : "h-auto"
            }
          >
            <div
              className={`flex items-center gap-2 px-3 py-2 ${isBottomOpen ? "border-b border-white/8" : ""}`}
            >
              <button
                onClick={() => {
                  setBottomTab("tests");
                ...
                className={`rounded-lg px-3 py-1.5 text-xs font-medium transition ${
                  bottomTab === "tests"
                    ? "bg-primary-200/20 text-primary-200"
                    : "text-light-400 hover:bg-white/5"
                }`}
              >
                Test Cases
              </button>
              ...
              >
                ...
                <ChevronDown
                  className={`h-3.5 w-3.5 transition-transform ${isBottomOpen ? "rotate-180" : ""}`}
                />
              </button>
            </div>
                  ...
                  <div className="rounded-xl border border-white/10 bg-white/5 p-3">
                    ...
                    {activeQuestion.problemType === "DESIGN" ? (
                      <p className="text-[11px] text-light-400 italic">
                        Custom input is not available for design problems. Use
                        the default test cases above.
                      </p>
                    ) : (
                      <>
                        <textarea
                        ...
                        <p className="mt-2 text-[11px] text-light-400">
                          ...
                          input in the same format as examples.
                        </p>
                      </>
                    )}
                  </div>
                  ...
                  <div className="rounded-xl border border-primary-200/20 bg-primary-200/8 p-3">
                    ...
                    {output ? (
                      <div className="space-y-3 text-xs text-light-100">
                        <p className="leading-5 text-light-100">
                        ...
                        <div>
                          <p className="mb-1 text-light-400">
                            Optimization Suggestions
                          </p>
                          <ul className="space-y-1">
                            {output.aiFeedback.optimizationSuggestions.map(
                              (suggestion) => (
                                <li
                                  key={suggestion}
                                  className="rounded-lg bg-white/5 px-2 py-1"
                      ...
                      <p className="text-xs text-light-300">
                        Submit your solution to receive complexity analysis,
                        optimization advice, and best-practice feedback.
                      </p>
                    )}
                  </div>
                </div>
              ...
              <div className="h-[calc(100%-43px)] overflow-y-auto p-3">
                <div className="rounded-xl border border-white/10 bg-dark-200/70 p-3">
                  ...
                  {submissionHistory.length === 0 ? (
                    <p className="text-xs text-light-400">
                      No submissions yet. Submit a solution to start tracking
                      attempts.
                    </p>
                  ) : (
                      ...
                      <table className="min-w-full text-left text-xs">
                        ...
                        <tbody>
                          {submissionHistory.map((item) => (
                            <tr
                              key={item.id}
                              className="border-b border-white/5 text-light-200"
                            >
                              ...
                              <td className="px-2 py-2">
                                <span
                                  className={`rounded-full px-2 py-0.5 text-[11px] ${getStatusClasses(item.status)}`}
                                >
                                  {item.status}
                                </span>
                              </td>
