...
import { VoiceRecorder } from "@/lib/speech/voiceRecorder";
import {
...
const InterviewInterface = ({ interview, userId }: Props) => {
  ...
  const [currentVoiceDurationSeconds, setCurrentVoiceDurationSeconds] =
    useState(0);
  ...
  const responseWordCount = currentResponse.trim()
    ? currentResponse.trim().split(/\s+/).length
    : 0;
  ...
  const handleNextQuestion = async () => {
    ...
    const speechCoach =
      currentVoiceDurationSeconds > 0
        ? analyzeSpeechCoachResponse(
            currentQuestion,
            currentResponse.trim(),
            currentVoiceDurationSeconds,
          )
        : undefined;
  ...
  const handleCompleteInterview = async (
    allResponses: InterviewResponse[] = responses,
  ) => {
    setIsLoading(true);
    try {
      ...
      const speechCoachAnalyses = allResponses
        ...
        .filter((analysis): analysis is SpeechCoachResponseInsight =>
          Boolean(analysis),
        );

      const speechCoachSummary =
        speechCoachAnalyses.length > 0
      ...
      speechSynthesizer.speak(
        `Interview completed! Your overall score is ${finalAnalysis.overallScore} out of 100. Performance level: ${finalAnalysis.performanceLevel}. Hiring recommendation: ${finalAnalysis.hiringRecommendation}. You can view your comprehensive detailed feedback now.`,
        {
          onEnd: () => {
            toast.success(
              `Interview completed! Score: ${finalAnalysis.overallScore}/100 - ${finalAnalysis.performanceLevel}`,
            );
  ...
  const avgWordsPerAnswer =
    answeredCount > 0
      ? Math.round(
          responses.reduce(
            (sum, response) =>
              sum + response.answer.split(/\s+/).filter(Boolean).length,
            0,
          ) / answeredCount,
        )
      : 0;
  ...
  return (
    <div className="interview-live-shell h-screen overflow-hidden bg-[radial-gradient(circle_at_12%_0%,rgba(59,130,246,0.16),transparent_28%),radial-gradient(circle_at_88%_12%,rgba(139,92,246,0.16),transparent_28%),linear-gradient(180deg,#070b16_0%,#0b1020_100%)] p-3 md:p-4">
      <div className="mx-auto flex h-full max-w-7xl flex-col gap-3">
        <section className="shrink-0 rounded-[28px] border border-white/12 bg-white/[0.04] px-4 py-3 backdrop-blur-xl shadow-[0_20px_60px_-36px_rgba(0,0,0,0.75)]">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div className="min-w-0">
              <p className="text-[10px] font-semibold uppercase tracking-[0.22em] text-primary-200/90">
                Interview Command Deck
              </p>
              <h1 className="truncate text-[1.55rem] font-semibold tracking-tight text-white md:text-[2rem]">
                {interview.role}
              </h1>
              <p className="mt-1 text-xs text-light-400">
                {interview.type} interview · {interview.level} level · no-scroll
                layout
              </p>
            </div>
          ...
          <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-white/10">
            <div
              className="h-1.5 rounded-full bg-gradient-to-r from-sky-400 via-primary-200 to-violet-400 transition-all duration-500"
        ...
        <div className="grid min-h-0 flex-1 gap-3 xl:grid-cols-[minmax(0,1fr)_270px] xl:items-stretch">
          <section className="grid h-full min-h-0 gap-3">
            <article className="interview-glass rounded-[28px] p-4 md:p-5">
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <Image
                    ...
                    width={38}
                    height={38}
                    className="rounded-full border border-primary-200/30"
                  />
                  <div>
                    <p className="text-[10px] uppercase tracking-[0.16em] text-primary-200">
                      Current Prompt
                    </p>
                ...
                <span
                  className={`inline-flex items-center gap-2 rounded-full border px-2.5 py-1 text-[10px] ${isSpeaking ? "border-amber-300/35 bg-amber-300/12 text-amber-200" : "border-emerald-300/30 bg-emerald-300/10 text-emerald-200"}`}
                >
                  <span
                    className={`h-2 w-2 rounded-full ${isSpeaking ? "bg-amber-300 animate-pulse" : "bg-emerald-300"}`}
                  />
            ...
            <article className="interview-glass rounded-[28px] p-2.5 md:p-3">
              <div className="mb-1 flex items-center justify-between gap-1.5">
                <div className="flex items-center gap-2">
                  ...
                  />
                  <div>
                    <p className="text-[9px] uppercase tracking-[0.16em] text-light-300">
                      Your Response
                    </p>
              ...
              <textarea
                ...
                className={`interview-textarea min-h-[3.25rem] md:min-h-[3.75rem] ${isRecording ? "interview-textarea-recording" : ""}`}
          ...
          <aside className="grid h-full min-h-0 gap-2 xl:grid-rows-[auto_1fr]">
            <article className="interview-glass rounded-[28px] p-3">
              ...
              <p className="mt-1.5 text-[13px] leading-relaxed text-light-100">
                Use one sharp example, one decision, and one measurable result.
              </p>
            </article>

            <article className="interview-glass flex h-full flex-col rounded-[28px] p-3">
              <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-light-300">
                Answer Coach
              </p>
              ...
              <div className="mt-2 flex-1 rounded-2xl border border-white/10 bg-black/20 p-2.5">
                ...
                <div className="mt-2 space-y-1.5 text-[11px] text-light-100">
                  <p className="flex items-center gap-2">
                    <span
                      className={`h-2 w-2 rounded-full ${responseWordCount >= 20 ? "bg-emerald-300" : "bg-amber-300"}`}
                    />
                    {responseWordCount >= 20
                      ? "Answer has enough detail"
                      : "Add a bit more detail"}
                  </p>
                  <p className="flex items-center gap-2">
                    <span
                      className={`h-2 w-2 rounded-full ${isSpeaking ? "bg-amber-300" : "bg-emerald-300"}`}
                    />
                    {isSpeaking
                      ? "Wait for AI to finish speaking"
                      : "Good timing to respond"}
                  </p>
                  <p className="flex items-center gap-2">
                    <span
                      className={`h-2 w-2 rounded-full ${isRecording ? "bg-primary-200 animate-pulse" : "bg-white/40"}`}
                    />
