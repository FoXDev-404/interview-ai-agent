...
\begin{itemize}
  \item \textbf{Chapter~\ref{ch:introduction}} introduces the background, motivation, problem statement, objectives, and scope.
  \item \textbf{Chapter~\ref{ch:literature}} presents a comprehensive literature survey of related work, existing systems, and identified research gaps.
  \item \textbf{Chapter~\ref{ch:design}} details the system architecture, module design, data models, and design patterns.
  \item \textbf{Chapter~\ref{ch:implementation}} describes the implementation methodology, module-wise development, algorithms, and technology integration.
  \item \textbf{Chapter~\ref{ch:results}} presents testing methodology, performance metrics, result analysis, and comparative evaluation.
  \item \textbf{Chapter~\ref{ch:conclusion}} summarizes achievements, contributions, limitations, and future enhancement roadmap.
\end{itemize}
...
The literature review examines five key domains relevant to AI MockPrep: (i)~AI-powered interview platforms, (ii)~Natural Language Processing for assessment, (iii)~speech analytics in professional communication, (iv)~resume optimisation and ATS technology, and (v)~real-time feedback systems. This survey identifies the individual strengths and collective limitations of existing approaches, establishing the research gap that AI MockPrep addresses.

\section{AI-Powered Interview Platforms}

\subsection{Final Round AI}
Final Round AI~\cite{ref_finalroundai} offers an avatar-based interview simulation with real-time conversational AI. The platform employs GPT-based models for dynamic question generation and provides feedback on communication style. However, Final Round AI focuses predominantly on behavioural interviews without technical customisation for specific technology stacks. The platform lacks resume analysis, speech analytics, and coding assessment capabilities, limiting its utility for comprehensive interview preparation.

\subsection{Google Interview Warmup}
Google Interview Warmup~\cite{ref_google_warmup} provides a minimalist interview practice tool with AI-evaluated responses. Users select from predefined career tracks (Data Analytics, UX Design, IT Support) and answer domain-specific questions. The system analyses responses for talking points, most-used words, and job-related terms. However, the platform offers only basic analysis without personalisation, adaptive difficulty, or quantitative scoring. There is no support for voice-based interviews, resume analysis, or coding assessment.

\subsection{Huru AI}
Huru AI~\cite{ref_huru} provides body language assessment through video analysis, evaluating eye contact, posture, and facial expressions during mock interviews. The platform offers valuable non-verbal feedback that most competitors lack. However, Huru AI does not integrate resume optimisation, speech analytics with quantitative metrics, or coding interview capabilities. The feedback is qualitative rather than rubric-based, limiting objective performance tracking.

\subsection{Pramp}
Pramp~\cite{ref_pramp} facilitates peer-to-peer mock interviews, matching candidates with other job seekers for mutual practice sessions. While this approach provides valuable interpersonal dynamics, it is fundamentally non-AI-driven, relies on the subjective assessment of untrained peers, and cannot guarantee availability, consistency, or domain expertise. The platform lacks resume analysis, speech analytics, and adaptive question generation.

\subsection{Teal AI}
Teal AI~\cite{ref_teal} focuses on resume building and job tracking with AI-assisted content suggestions. The platform offers ATS-friendly templates and keyword optimisation. However, Teal AI lacks immersive interview simulation environments, emotional/sentiment feedback, and speech analytics capabilities, functioning primarily as a career management tool rather than a comprehensive preparation platform.

\section{NLP in Interview Assessment}

\subsection{Transformer-Based Models for Question Generation}
Lee~\cite{ref_lee2024} demonstrated that transformer-based models, particularly those fine-tuned on domain-specific corpora, can generate contextually appropriate interview questions with 89\% relevance ratings from human evaluators. The study employed Mistral-7B-Instruct with role-specific prompt engineering, achieving superior question diversity compared to template-based approaches.

\subsection{Sentiment Analysis in Professional Communication}
Kumar et al.~\cite{ref_kumar2024} developed a real-time sentiment analysis framework for evaluating interview responses. Their approach achieved 86\% accuracy in classifying candidate confidence levels using a combination of linguistic features (hedging patterns, assertive language) and prosodic cues (pitch variation, speech rate). The framework, however, required dedicated training on interview-specific datasets, limiting its generalisability.

\subsection{Conversational AI and Dynamic Questioning}
Zhang and Rodriguez~\cite{ref_zhang2023} explored transformer-based models for context-aware conversational understanding in interview settings. Their approach maintained coherent multi-turn conversations, dynamically adapting questions based on candidate responses. The research highlighted the importance of maintaining question context across a session, though scalable deployment remained a challenge.

\section{Speech Analytics}

\subsection{Real-Time Speech Processing}
Chen~\cite{ref_chen2024} demonstrated that machine learning models can achieve 91\% accuracy in real-time speech-to-text transcription when combined with word-level timestamp alignment. The study used AssemblyAI's universal model for transcription and developed custom algorithms for computing WPM segments, filler word detection, and hesitation pause identification.

\subsection{Filler Word Detection}
Research by Martinez and Brown~\cite{ref_martinez2024} established that automated filler word detection with frequency tracking provides actionable feedback that reduces filler usage by an average of 41\% over six practice sessions. Their system detected common fillers (um, uh, like, basically, actually, literally) and phrasal fillers (you know, kind of, sort of, I mean) with 94\% precision.

\section{Resume Optimisation and ATS Technology}

\subsection{Keyword-Based ATS Filtering}
Rodriguez and Kim~\cite{ref_rodriguez2024} analysed the algorithmic filtering strategies employed by major ATS vendors (Taleo, Greenhouse, Lever, Workday). Their study found that keyword matching accounts for approximately 30\% of the filtering decision, semantic alignment contributes 25\%, and format compliance accounts for 10\%. This empirical distribution informed the composite scoring weights adopted in AI MockPrep.

\subsection{AI-Generated Bullet Point Rewrites}
Recent work by Smith and Johnson~\cite{ref_smith2024} demonstrated that AI-generated resume bullet point rewrites, incorporating action verbs, quantifiable metrics, and impact-oriented language, improve recruiter response rates by 38\%. Their approach used large language models to transform weak, passive bullet points into strong, metrics-driven statements.
...
The literature survey reveals the following critical gaps:

\begin{enumerate}
  \item \textbf{No Unified Platform:} No existing system combines interview simulation, real-time rubric-based evaluation, speech analytics, ATS resume analysis, coding assessment, and a resume builder within a single platform.
  \item \textbf{Absence of Interview-Type-Aware Scoring:} Existing platforms apply uniform evaluation criteria regardless of interview type. Interview-type-specific weighting (e.g., STAR structure for behavioural, correctness for coding) is absent.
  \item \textbf{Limited Quantitative Speech Analytics:} While speech transcription exists, comprehensive quantitative metrics (WPM segments, pacing consistency, filler frequency per 100 words, adaptive hesitation thresholds) are not available in interview preparation tools.
  \item \textbf{No Composite ATS Scoring:} Resume analysis tools either focus on keyword matching or format compliance, but do not provide composite scoring models that mirror actual ATS evaluation strategies.
  \item \textbf{Lack of Coding Assessment Integration:} Interview platforms either focus on verbal interviews or coding challenges, but not both within a unified workflow.
\end{enumerate}

AI MockPrep addresses all five gaps by providing a unified, accessible, AI-driven platform that integrates all essential preparation modules.
...
AI MockPrep was developed using an \textbf{Agile} methodology with iterative sprint cycles of two weeks each. The development process followed these phases:
