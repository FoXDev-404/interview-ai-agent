...
% ── Header/Footer ─────────────────────────────────────────
% Default style for main-body pages (arabic numbering)
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textit{AI MockPrep}}   % Left header: project short title
\fancyhead[R]{\small\nouppercase{\leftmark}} % Right header: current chapter name
\fancyfoot[C]{\thepage}                      % Centre footer: page number
\renewcommand{\headrulewidth}{0.4pt}         % Rule below header
\renewcommand{\footrulewidth}{0.4pt}         % Rule above footer

% Plain style – used automatically on the first page of every \chapter
% Shows page number at bottom centre; no header or header rule
...
% Frontmatter style – roman-numbered preliminary pages
% Shows page number at bottom centre; no header
